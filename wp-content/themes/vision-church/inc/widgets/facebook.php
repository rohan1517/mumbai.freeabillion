...
class vision_church_facebook_widget extends WP_Widget{
	function __construct(){
		$params = array('description'=> 'Your recently posts from facebook will be displayed','name'=> 'Webnus - Facebook');
		parent::__construct('vision_church_facebook_widget', '', $params);
	}
	public function form($instance){
		...
		<p><label for="<?php echo esc_attr( $this->get_field_id('url') ); ?>"><?php esc_html_e('Page Address:','vision-church') ?></label><input type="text"	class="widefat"	id="<?php echo esc_attr( $this->get_field_id('url') ); ?>"	name="<?php echo esc_attr( $this->get_field_name('url') ); ?>"	value="<?php if( isset($url) )  echo esc_attr($url); ?>" /><small><?php esc_html_e( 'Your Page address', 'vision-church' ); ?></small></p>			
		<?php 
	}
...
add_action('widgets_init', 'register_vision_church_facebook'); 
function register_vision_church_facebook(){
	register_widget('vision_church_facebook_widget');
}