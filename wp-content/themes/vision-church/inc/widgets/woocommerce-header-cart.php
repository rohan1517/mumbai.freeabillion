<?php class Woocommerce_Header_Cart extends WP_Widget {
	public function __construct() {
		parent::__construct('woocommerce-header-cart','Woocommerce Header Cart',
			array( 'description' => esc_html__( 'Woocommerce Header Cart', 'vision-church' ), )
		);
	}
				...
				?>
					<ul class="<?php echo implode(' ', $list_class); ?>">
						...
						foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $cart_item ) :
							...
							<?php endforeach; ?>
						<?php else : ?>
							<li><?php esc_html_e( 'No products in the cart.', 'vision-church' ); ?></li>
						<?php endif; ?>
					</ul>
			<?php if ( sizeof( $woocommerce->cart->get_cart() ) <= 0 ) : ?>
			<?php endif; ?>
                <a href="<?php echo esc_url($woocommerce->cart->get_cart_url()); ?>" class="qbutton white view-cart"><?php esc_html_e( 'Cart', 'vision-church' ); ?> </a>
                    <span class="total"><?php esc_html_e( 'Total', 'vision-church' ); ?>:<span><?php echo '' . $woocommerce->cart->get_cart_subtotal(); ?></span></span>
			<?php if ( sizeof( $woocommerce->cart->get_cart() ) <= 0 ) : ?>
			<?php endif; ?>
	</div>
</div>
