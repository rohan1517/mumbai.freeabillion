...
class WebnusInstagramWidget extends WP_Widget{
	...
	public function form($instance){
		...
		<p><label for="<?php echo esc_attr( $this->get_field_id('type') ) ?>"><?php esc_html_e('Recent media published by:','vision-church') ?></label><select id="<?php echo esc_attr( $this->get_field_id('type') ); ?>" name="<?php echo esc_attr( $this->get_field_name('type') ); ?>" class="widefat" style="width:100%;"><option <?php if ('Username' == $instance['type']) echo 'selected="selected"'; ?>>Username</option><option <?php if ('Hashtag' == $instance['type']) echo 'selected="selected"'; ?>>Hashtag</option>		</select></p>
		<p><label for="<?php echo esc_attr( $this->get_field_id('username') ) ?>"><?php esc_html_e('Instagram Username/Hashtag:','vision-church') ?></label><input type="text" class="widefat" id="<?php echo esc_attr( $this->get_field_id('username') ) ?>" name="<?php echo esc_attr( $this->get_field_name('username') ) ?>" value="<?php if( isset($username) )  echo esc_attr($username); ?>" /></p>
		<p><label for="<?php echo esc_attr( $this->get_field_id('token') ) ?>"><?php esc_html_e('Instagram Access Token:','vision-church') ?></label><input type="text" class="widefat" id="<?php echo esc_attr( $this->get_field_id('token') ) ?>" name="<?php echo esc_attr( $this->get_field_name('token') ) ?>" value="<?php if( isset($token) ) echo esc_attr($token); ?>" /><small>Get the this information <a target="_blank" href="http://www.pinceladasdaweb.com.br/instagram/access-token/">here</a>.</small></p>
		<p><label for="<?php echo esc_attr( $this->get_field_id('count') ) ?>"><?php esc_html_e('Feed Count(Max 20):','vision-church') ?></label><input type="text" class="widefat" id="<?php echo esc_attr( $this->get_field_id('count') ) ?>" name="<?php echo esc_attr( $this->get_field_name('count') ) ?>" value="<?php if( isset($count) )  echo esc_attr($count); ?>" /></p>
	<?php }
	public function widget($args, $instance){
		...
		if(!empty($username) && !empty($token) ){ ?>
			...
			if(!is_wp_error($raw_content)){
				...
				if(!is_wp_error($raw_content)){
					...
					if (isset($json_insta->data[0])){
						foreach($json_insta->data as $data){		
							$output .= '<li><a href="'.$data->link.'" target="_blank"><img alt="'.$username.'" src="'.$data->images->thumbnail->url.'"/></a></li>';
						}
					}
					$output .= '</ul>';	
					echo '' . $output;
				...
				echo esc_html__('An error has occoured...','vision-church');
			?>
			<div class="clear"></div>
			</div>	 
...
add_action('widgets_init','register_vision_church_instagram_widget'); 
function register_vision_church_instagram_widget(){
