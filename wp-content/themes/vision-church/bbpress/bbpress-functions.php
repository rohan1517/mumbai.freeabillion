...
class BBP_Default extends BBP_Theme_Compat {
	...
	private function setup_globals() {
		$bbp           = bbpress();
		$this->id      = 'default';
		$this->name    = esc_html__( 'bbPress Default', 'vision-church' );
	...
	public function localize_topic_script() {
		...
		wp_localize_script( 'bbpress-topic', 'bbpTopicJS', array(
			...
			'generic_ajax_error' => esc_html__( 'Something went wrong. Refresh your browser and try again.', 'vision-church' ),
	...
	public function ajax_favorite() {
		...
		if ( ! bbp_is_favorites_active() ) {
			bbp_ajax_response( false, esc_html__( 'Favorites are no longer active.', 'vision-church' ), 300 );
		}

		// Bail if user is not logged in
		if ( !is_user_logged_in() ) {
			bbp_ajax_response( false, esc_html__( 'Please login to make this topic a favorite.', 'vision-church' ), 301 );
		}
		...
		if ( !current_user_can( 'edit_user', $user_id ) ) {
			bbp_ajax_response( false, esc_html__( 'You do not have permission to do this.', 'vision-church' ), 302 );
		}
		...
		if ( empty( $topic ) ) {
			bbp_ajax_response( false, esc_html__( 'The topic could not be found.', 'vision-church' ), 303 );
		}

		// Bail if user did not take this action
		if ( !isset( $_POST['nonce'] ) || !wp_verify_nonce( $_POST['nonce'], 'toggle-favorite_' . $topic->ID ) ) {
			bbp_ajax_response( false, esc_html__( 'Are you sure you meant to do that?', 'vision-church' ), 304 );
		}
		...
		if ( empty( $status ) ) {
			bbp_ajax_response( false, esc_html__( 'The request was unsuccessful. Please try again.', 'vision-church' ), 305 );
		}
	...
	public function ajax_subscription() {
		...
		if ( !bbp_is_subscriptions_active() ) {
			bbp_ajax_response( false, esc_html__( 'Subscriptions are no longer active.', 'vision-church' ), 300 );
		}

		// Bail if user is not logged in
		if ( !is_user_logged_in() ) {
			bbp_ajax_response( false, esc_html__( 'Please login to subscribe to this topic.', 'vision-church' ), 301 );
		}
