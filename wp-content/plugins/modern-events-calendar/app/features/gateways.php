...
class MEC_gateway extends MEC_base implements MEC_gateway_interface
{
    ...
    public function comment()
    {
        return ((isset($this->options['comment']) and trim($this->options['comment'])) ? '<p class="mec-gateway-comment">'.__($this->options['comment'], 'mec').'</p>' : '');
    }
...
class MEC_gateway_stripe extends MEC_gateway
{
    ...
    public function do_transaction($transaction_id = NULL)
    {
        ...
        $redirect_to = '';
        if(isset($this->settings['booking_thankyou_page']) and trim($this->settings['booking_thankyou_page'])) $redirect_to = get_permalink($this->settings['booking_thankyou_page']);
        
        $this->response(array('success'=>1, 'message'=>__('Thanks for your booking. Your tickets booked, booking verification might be needed, please check your email.', 'mec'), 'data'=>array('book_id'=>$book_id, 'redirect_to'=>$redirect_to)));
    }
    ...
    ?>
        ...
        <div id="mec_gateways<?php echo $this->id(); ?>_container_toggle" class="mec-gateway-options-form <?php if((isset($this->options['status']) and !$this->options['status']) or !isset($this->options['status'])) echo 'mec-util-hidden'; ?>">
            ...
            <div class="mec-form-row">
                ...
                <div class="mec-col-4">
                    <textarea id="mec_gateways<?php echo $this->id(); ?>_comment" name="mec[gateways][<?php echo $this->id(); ?>][comment]"><?php echo (isset($this->options['comment']) and trim($this->options['comment'])) ? $this->options['comment'] : ''; ?></textarea>
...
class MEC_gateway_paypal_express extends MEC_gateway
{
    ...
    public function get_api_url()
    {
        ...
        elseif($this->options['mode'] == 'sandbox') $url = $sandbox;
    ...
    public function check_transaction($transaction_id = NULL)
    {
        ...
        if(isset($transaction['done']) and $transaction['done'] == 1)
        {
            ...
            if(isset($this->settings['booking_thankyou_page']) and trim($this->settings['booking_thankyou_page'])) $data['redirect_to'] = get_permalink($this->settings['booking_thankyou_page']);
        }
...
class MEC_gateway_woocommerce extends MEC_gateway
{
    ...
    public function create_order()
    {
        ...
        update_post_meta($order->id, '_mec_transaction_id', $transaction_id);
        ...
        if(trim($ex[0])) update_post_meta($order->id, '_billing_first_name', $ex[0]);
        if(trim($ex[1])) update_post_meta($order->id, '_billing_last_name', implode(' ', array_slice($ex, 1)));
        ...
        $this->response(array('success'=>1, 'message'=>__('Your order is created. Please proceed with checkout.', 'mec'), 'data'=>array('url'=>$url, 'id'=>$order->id)));
    }
    ...
    public function do_transaction($transaction_id = NULL, $order_id = NULL)
    {
        ...
        if($order_id)
        {
            ...
            if(!$customer) update_post_meta($order_id, '_customer_user', $first_user_id);
        }
        
        $redirect_to = '';
        if(isset($this->settings['booking_thankyou_page']) and trim($this->settings['booking_thankyou_page'])) $redirect_to = get_permalink($this->settings['booking_thankyou_page']);
        
        return array('success'=>1, 'message'=>__('Thanks for your booking. Your tickets booked, booking verification might be needed, please check your email.', 'mec'), 'data'=>array('book_id'=>$book_id, 'redirect_to'=>$redirect_to));
    }
    ...
    public function checkout_form($transaction_id, $params = array())
    {
        ...
        <form id="mec_do_transaction_woocommerce_form<?php echo $transaction_id; ?>">
            <div class="mec-form-row">
                ...
                <input type="hidden" name="gateway_id" value="<?php echo $this->id(); ?>" />
                <?php wp_nonce_field('mec_transaction_form_'.$transaction_id); ?>
                <button type="submit"><?php echo __('Order', 'mec'); ?></button>
            </div>
        </form>
