...
class MEC_main extends MEC_base
{
    ...
    {
        $skins = array('list'=>__('List View', 'mec'), 'grid'=>__('Grid View', 'mec'), 'carousel'=>__('Carousel View', 'mec'), 'full_calendar'=>__('Full Calendar', 'mec'), 'monthly_view'=>__('Calendar/Monthly View', 'mec'), 'daily_view'=>__('Daily View', 'mec'), 'weekly_view'=>__('Weekly View', 'mec'), 'map'=>__('Map View', 'mec'), 'cover'=>__('Cover View', 'mec'), 'countdown'=>__('Countdown View', 'mec'),'slider'=>__('Slider View', 'mec'));
    ...
    {
        $week_start = $this->get_first_day_of_week();
        $raw = array(__('SU', 'mec'), __('MO', 'mec'), __('TU', 'mec'), __('WE', 'mec'), __('TH', 'mec'), __('FR', 'mec'), __('SA', 'mec'));
    ...
    {
        $options = $this->get_options();
        return (isset($options['reg_fields']) ? $options['reg_fields'] : array());
    }
		...
		<div class="mec-wrap">
			<div class="mec-map-lightbox-wp mec-event-list-classic">
				<article class="mec-event-article mec-clear">
					'.$infowindow_thumb.'
					<a href="'.$this->get_event_date_permalink($event->data->permalink, $event->date['start']['date']).'"><div class="mec-event-date mec-color"><i class="mec-sl-calendar"></i> '.$this->date_label($event->date['start'], $event->date['end'], $date_format).'</div></a>
					<h4 class="mec-event-title"><a class="mec-color-hover" href="'.$this->get_event_date_permalink($event->data->permalink, $event->date['start']['date']).'">'.$event->data->title.'</a></h4>
				</article>
			</div>
		</div>';
    ...
    public function ical_single($event_id, $occurrence = '')
    {
        ...
        $ical .= "SUMMARY:".$event->title.PHP_EOL;
        $ical .= "DESCRIPTION:".trim(strip_tags($event->content)).PHP_EOL;
    ...
    public function date_label($start, $end, $format, $separator = ' - ')
    {
        ...
        elseif($start_timestamp < $end_timestamp) return date_i18n($format, $start_timestamp).$separator.date_i18n($format, $end_timestamp);
    }
    ...
    public function load_map_assets()
    {
        ...
        // Include Lity Lightbox for using in Map views
        wp_enqueue_script('mec-lity-script', $this->asset('packages/lity/lity.min.js'));
        
        // Include Google Maps Javascript API
        $gm_include = apply_filters('mec_gm_include', true);
        if($gm_include) wp_enqueue_script('googlemap', '//maps.googleapis.com/maps/api/js'.((isset($settings['google_maps_api_key']) and trim($settings['google_maps_api_key']) != '') ? '?key='.$settings['google_maps_api_key'] : ''));
        ...
        wp_enqueue_style('mec-lity-style', $this->asset('packages/lity/lity.min.css'));
    }
    ...
    public function get_timezone_by_ip()
    {
        ...
        // Second Provider
        if(!trim($JSON)) $JSON = $this->get_web_page('https://ipapi.co/'.$ip.'/json/');
        
        $data = json_decode($JSON, true);
        return isset($data['timezone']) ? $data['timezone'] : false;
    }
}