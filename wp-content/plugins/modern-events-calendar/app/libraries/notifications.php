...
class MEC_notifications extends MEC_base
{
    ...
    public function email_verification($book_id)
    {
        ...
        // Send the mail
        wp_mail($to, $subject, wpautop(stripslashes($message)), $headers);
    }
    ...
    public function booking_notification($book_id)
    {
        ...
        $to = $booker->user_email;
        $subject = isset($this->notif_settings['booking_notification']['subject']) ? __($this->notif_settings['booking_notification']['subject'], 'mec') : __('Please verify your email.', 'mec');
        $headers = array();
    ...
    public function new_event($event_id, $post, $update)
    {
        ...
        if(get_post_type($event_id) != $event_PT) return;
        
        // If it's an update request, then don't send any notification
        if($update) return;
        
        // New event notification is disabled
        if(!isset($this->notif_settings['new_event']['status']) or (isset($this->notif_settings['new_event']['status']) and !$this->notif_settings['new_event']['status'])) return;
    ...
    public function content($message, $book_id)
    {
        ...
        // Booker Data
        $message = str_replace('%%first_name%%', (isset($booker->first_name) ? $booker->first_name : ''), $message);
        $message = str_replace('%%last_name%%', (isset($booker->last_name) ? $booker->last_name : ''), $message);
