...
class MEC_book extends MEC_base
{
    ...
    public function get_price_details($tickets, $event_id, $event_tickets)
    {
        ...
        $details[] = array('amount'=>$total, 'description'=>__('Tickets Price', 'mec'), 'type'=>'tickets');
    ...
    public function add($values, $transaction_id, $ticket_id)
    {
        ...
        if(isset($transaction['coupon']))
        {
            ...
            if($coupon_id) wp_set_object_terms($book_id, $coupon_id, 'mec_coupon');
        }
    ...
    public function get_tickets_availability($event_id, $date)
    {
        ...
        foreach($tickets as $ticket_id=>$ticket)
        {
            ...
            // Unlimited Total
            if($total_bookings_limit == '-1') $availability[$ticket_id] = $limit-$query->post_count;
            else $availability[$ticket_id] = min(($limit-$query->post_count), max($total_bookings_limit, 0));
     ...
     * @return int
     */
    public function coupon_check_validity($coupon)
    {
        ...
        return (($usage_limit == '-1' or (int) $usage_limit > $term->count) ? 1 : -1);
    }
    ...
    public function coupon_apply($coupon, $transaction_id)
    {
        ...
        if($this->coupon_check_validity($coupon) != 1) return 0;
