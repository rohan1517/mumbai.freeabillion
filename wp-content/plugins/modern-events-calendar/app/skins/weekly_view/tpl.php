...
foreach($this->weeks as $week_number=>$week)
{
    ...
    foreach($week as $day)
    {
        ...
        $weeks .= '<dt data-events-count="'.$count.'" class="'.((strtotime($day) < $current_month_time or strtotime($day) >= $_1month_after)? 'mec-weekly-disabled' : '').'">'
                ...
                //.'<span class="mec-weekly-view-event-count">'.$count.'</span>'
                .'</dt>';
    }
...
if($this->next_previous_button)
{
    ...
    <div class="mec-next-month mec-load-month mec-color" data-mec-year="'.date('Y', $_1month_after).'" data-mec-month="'.date('m', $_1month_after).'"><i class="mec-sl-angle-right"></i></div>';
}
