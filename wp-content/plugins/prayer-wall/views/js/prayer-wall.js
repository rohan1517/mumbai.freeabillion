...
$(document).ready(function($) {
	...
	// Start PrayBox
		...
		$('.pbx-formfield input[type="text"], .pbx-formfield textarea, .pbx-formfield label').on('click', function() {
			var $this = $(this).closest('.pbx-formfield').find('label');
			$this.css('margin-top', '-20px');
			$(this).closest('.pbx-formfield').addClass('pbx-active');
		});

		$('.pbx-formfield input[type="text"], .pbx-formfield textarea, .pbx-formfield label').focusout(function(event) {
			var $this = $(this).closest('.pbx-formfield').find('label');
			if ( $(this).val() == '' ) {
