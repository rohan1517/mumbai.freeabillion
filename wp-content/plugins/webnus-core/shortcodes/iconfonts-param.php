...
function vision_church_icomoon_composer($settings, $value) {

   $out = '<input id="vision_church_icomoon_textbox" name="'.$settings['param_name'].'" class="wpb_vc_param_value  '
   .$settings['param_name'].' '.$settings['type'].'_field" type="hidden" value="'
             .$value.'">';

	$out .= "<script>jQuery(document).ready(function() { jQuery('input[name=\"iconfonts_name\"]').click(function(){ jQuery('#vision_church_icomoon_textbox').val(jQuery(this).val()) ; });  } );</script>";
   $out .= '<div id="retinaicon-form" class="webnus-iconfonts-wrapper">
	...
	</div><script type="text/javascript">jQuery(document).ready(function(){  var val = jQuery("#vision_church_icomoon_textbox").val();  jQuery("#"+val).trigger("click"); });</script>';

	return $out;
}

if ( class_exists( 'Vc_Manager' ) ) :
	vc_add_shortcode_param('iconfonts', 'vision_church_icomoon_composer');
endif;