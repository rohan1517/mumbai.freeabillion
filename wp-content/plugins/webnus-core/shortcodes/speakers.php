...
function webnus_speakers( $attributes, $content = null ) {
	...
	foreach( $terms as $term ) {
		...
		if ( $sermons_count > 1 ) {
		$w_sermons_count = '<a href="'. get_term_link( $term ) .'">'. str_replace( '%', number_format_i18n( $sermons_count ), __( '% Sermons', 'webnus-core' )) .'</a>';
		} elseif ( $sermons_count == 0 ) {
		$w_sermons_count = __( 'No Sermons', 'webnus-core' );
		} else {
		$w_sermons_count = '<a href="'. get_term_link( $term ) .'">'. __( '1 Sermon', 'webnus-core' ) .'</a>';
		}
